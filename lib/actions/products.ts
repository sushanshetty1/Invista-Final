...
export async function createProduct(
	...
	try {
		...
		// 🏷️ Auto-create/update brand entry in brands table
		try {
			if (validatedData.brandName && productData.companyId) {
				console.log("🏭 Starting brand creation process...");
				console.log("🏷️ Brand name:", validatedData.brandName);
				console.log("🏢 Company ID:", productData.companyId);

				// Step 1: Get company info from Supabase
				...
				const { data: company, error: companyError } = await supabase
					.from("companies")
					.select("website, email, phone")
					.eq("id", productData.companyId)
					.single();

				if (companyError) {
					console.warn("⚠️ Error fetching company info:", companyError.message);
				}

				console.log("✅ Company info:", company);

				// Step 2: Check if brand already exists
				let brand = await neonClient.brand.findFirst({
					where: { name: validatedData.brandName }
				});

				if (brand) {
					console.log("📝 Updating existing brand:", brand.id);
					brand = await neonClient.brand.update({
						where: { id: brand.id },
						data: {
							description: product.description,
							website: company?.website || brand.website,
							contactEmail: company?.email || brand.contactEmail,
							contactPhone: company?.phone || brand.contactPhone,
					...
					console.log("📝 Creating new brand:", validatedData.brandName);
					brand = await neonClient.brand.create({
						data: {
				...
				console.log("🎉 Brand process completed successfully!");
			} else {
				console.log("⏭️ Skipping brand creation - no brandName or companyId");
			}
		} catch (brandError) {
			console.error("❌ Error managing brand:", brandError);
			if (brandError instanceof Error) {
				console.error("💥 Brand error message:", brandError.message);
				console.error("📚 Brand error stack:", brandError.stack);
			}
			// Don't fail the product creation if brand management fails
			console.warn("⚠️ Product created but brand management failed");
		}
