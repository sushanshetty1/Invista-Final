...
const ACCESS_LEVELS = [
  { value: 'READ_ONLY', label: 'Read Only', description: 'Can view location data' },
  { value: 'STANDARD', label: 'Standard', description: 'Can perform standard operations' },
  { value: 'SUPERVISOR', label: 'Supervisor', description: 'Can supervise operations' },
  { value: 'MANAGER', label: 'Manager', description: 'Can manage location settings' },
...
export default function UserLocationAssignment({ companyId }: UserLocationAssignmentProps) {
  ...
  const fetchUserLocationAccess = async (userId: string) => {
    try {
      const response = await fetch(`/api/companies/${companyId}/users/${userId}/location-access`);
  ...
  const handleAssignPrimaryLocation = async (userId: string, locationId: string) => {
    try {
      const response = await fetch(`/api/companies/${companyId}/users/${userId}/primary-location`, {
        method: 'PUT',
        headers: {
  ...
  const handleGrantLocationAccess = async (userId: string, locationId: string, accessLevel: string, canManage: boolean) => {
    try {
      const response = await fetch(`/api/companies/${companyId}/users/${userId}/location-access`, {
        method: 'POST',
        headers: {
      ...
      {selectedUser && (
        <Dialog open={isAssignDialogOpen} onOpenChange={setIsAssignDialogOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Location Access - {selectedUser.user.displayName || selectedUser.user.email}</DialogTitle>
              <DialogDescription>
...
function PrimaryLocationForm({ 
  ...
  const [selectedLocationId, setSelectedLocationId] = useState(user.primaryLocationId || '');
...
function LocationAccessManager({
  ...
  onGrantAccess: (userId: string, locationId: string, accessLevel: string, canManage: boolean) => void;
  companyId: string;
}) {
  ...
  const availableLocations = locations.filter(location => 
    !userLocationAccess.some(access => access.locationId === location.id && access.isActive)
  );
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Allow user to manage location settings</p>
            </div>
