	...
	useEffect(() => {
		if (typeof window !== "undefined" && !hasCompanyAccess && user) {
			...
			if (hasBeenGrantedAccess || inviteAccepted) {
				...
				console.log(
					"DashboardGuard - found access flag, refreshing access check",
				);
			...
			if (!loading) {
				if (!user) {
					...
					if (typeof window !== "undefined") {
						const hasBeenGrantedAccess = user?.id
							? localStorage.getItem(`invista_has_access_${user.id}`) === "true"
							: false;
						const inviteAccepted =
							localStorage.getItem("invista_invite_accepted") === "true";
						...
						if (hasBeenGrantedAccess || inviteAccepted) {
							console.log(
								"DashboardGuard - user had previous access, not redirecting to waiting",
							);
					...
					console.log(
						"DashboardGuard - no previous access, redirecting to waiting",
					);
					setIsRedirecting(true);
					router.push("/waiting");
				}
