...
export function useCompanyData(): UseCompanyDataResult {
  ...
  const updateCompany = useCallback(async (updateData: Partial<CompanyProfile>): Promise<boolean> => {
  ...
  const inviteUsers = useCallback(async (emails: string, role: string): Promise<boolean> => {
