    ...
    const fetchUserIndustry = async () => {
      ...
      try {
        ...
        const getAuthToken = async () => {
          ...
          let token = localStorage.getItem('sb-access-token') || sessionStorage.getItem('sb-access-token');
          
          // If not found, try to get from document cookies
          if (!token) {
            const cookies = document.cookie.split(';');
            const authCookie = cookies.find(cookie => cookie.trim().startsWith('sb-access-token='));
            if (authCookie) {
        ...
        console.error('Error fetching user industry:', err);        setError('Failed to load company industry');
        setIndustry(null);
        setCategories([]);
      } finally {
        setLoading(false);
