...
export async function GET(request: NextRequest) {
	try {
        ...
        `)
			...
			if (
				authError ||
				(!companyUser?.isOwner && companyUser?.role !== "ADMIN")
			) {
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const { companyId, emails, role, invitedById, invitedByName, message } =
			body;
		...
		if (!companyId || !emails || !role || !invitedById) {
			...
			return NextResponse.json(
				{
					error:
						"Missing required fields: companyId, emails, role, invitedById",
				},
				{ status: 400 },
			);
		...
		console.log(
			"Checking company authorization for user:",
			invitedById,
			"company:",
			companyId,
		);
		...
		if (!hasPermission) {
			return NextResponse.json(
				{
					error:
						"Unauthorized to send invites. You must be a company owner or admin.",
				},
				{ status: 403 },
			);
      ...
      `)
		...
		const existingEmails = new Set([
			...
			...(existingUsers || [])
				.map(
					(userRecord: { user: { email: string }[] | { email: string } }) => {
