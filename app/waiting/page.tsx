	...
	const fetchInvites = useCallback(async () => {
		...
		try {
			const response = await fetch(
				`/api/company-invites?email=${encodeURIComponent(user.email)}`,
			);
		...
		try {
			...
			if (response === "ACCEPTED") {
				...
				if (user?.id) {
					...
					localStorage.setItem(
						`invista_access_source_${user.id}`,
						"invite_acceptance",
					);
				}
								...
								<CardContent className="text-center space-y-6">
									<div className="bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-6">
										<p className="text-yellow-800 dark:text-yellow-200 font-medium">
											Please contact your company administrator to send you an
											invitation to join their Invista workspace.
										</p>
									</div>
